...
nv.models.focus = function(model) {
  "use strict";
  ...
  var content = model || nv.models.line()
    , xAxis = nv.models.axis()
    , yAxis = nv.models.axis()
    , brush = d3.svg.brush()
  ;
  ...
  content.interactive && content.interactive(false);
  ...
  function chart(selection) {
    ...
    selection.each(function(data) {
      ...
      x.domain(d3.extent(seriesData.map(function(d) { return d.x; }))).range([0, availableWidth]);
  ...
  chart._options = Object.create({}, {
    ...
    focusXScale:  {get: function(){return x;}, set: function(_){xAxis.scale(_);}},
  });
  ...
  function chart(selection) {
    ...
    selection.each(function(data) {
      ...
      function onBrush(extent) {
        ...
        linesWrap.transition().duration(duration).call(lines);
...
nv.models.linePlusBarChart = function() {
  ...
  var margin = {top: 30, right: 30, bottom: 30, left: 60}
    ...
    , y4
    , noData = null
    , dispatch = d3.dispatch('brush', 'tooltipHide', 'stateChange', 'changeState', 'renderEnd')
        ...
        interactiveLayer.dispatch.on('elementMousemove', function(e) {
          ...
          data
            ...
            .forEach(function(series,i) {
              ...
              if (pointXLocation === undefined) pointXLocation = chart.xScale()(chart.x()(point,pointIndex));
              allData.push({
                yAxis: series.bar ? getBarsAxis().main : getLinesAxis().main,
                key: series.key,
                value: chart.y()(point, pointIndex),
                color: color(series,series.seriesIndex),
                data: series.values[pointIndex]
          ...
          interactiveLayer.tooltip
            .valueFormatter(function(d,i) {
              return allData[i].yAxis.tickFormat()(d);
            })
            .data({
              value: xValue,
              index: pointIndex,
  ...
  chart._options = Object.create({}, {
    ...
    xScale:  {get: function(){return x;}, set: function(_){
        lines.xScale(_);
        bars.xScale(_);
        x2Axis.scale(_);
      }},
    ...
    useInteractiveGuideline: {get: function(){return useInteractiveGuideline;}, set: function(_){
        ...
        if (_ === true) {
          // chart.interactive(false);
          lines.interactive(false);
          lines.pointActive(function(d) { return false });
        }
      }}
  });
...
nv.models.multiBarChart = function() {
  ...
  var multibar = nv.models.multiBar()
    ...
    , controls = nv.models.legend()
    , tooltip = nv.models.tooltip()
    , focus = nv.models.focus(nv.models.multiBar())
  ;
  ...
  function chart(selection) {
    ...
    selection.each(function(data) {
      ...
      // Update Axes
      //============================================================
      function updateXAxis(brushData) {
        if(showXAxis) {
          g.select('.nv-focus .nv-x.nv-axis')
            .attr('transform', 'translate(0,' + y.range()[0] + ')');
          g.select('.nv-focus .nv-x.nv-axis')
          ...
          if (reduceXTicks) {
            xTicks
              .filter(function(d,i) {
                return i % Math.ceil(brushData[0].values.length / (availableWidth / 100)) !== 0;
              })
              .selectAll('text, line')
              .style('opacity', 0);
          }
      ...
      function onBrush(extent) {
        // Update Main (Focus)
        var barsWrap = g.select('.nv-focus .nv-barsWrap');
        var brushData = data.filter(function(d) { return !d.disabled; })
          .map(function(d,i) {
            return {
              key: d.key,
              area: d.area,
        ...
        barsWrap.datum(brushData).transition().duration(duration).call(multibar);

        // Update Main (Focus) Axes
        updateXAxis(brushData);
        updateYAxis();
      }
